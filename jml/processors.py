...
class JMLProcessor:
    ...
    def __init__(self,okta_client:OktaClient,group_manager:GroupManager,app_manager:AppManager):
    ...
    def process_hire(self,event:HREvent):
        ...
        user=self.okta.create_user(profile=event.to_okta_profile() ,activate=event.start_date is None or event.start_date<=date.today())
    ...
    def process_move(self,event:HREvent):
        ...
        for g_name in to_remove:
            ...
            if g:
                self.groups.delete_user_from_group(user["id",g["id"]])
